...
const calculateBalancedLines = (words: string[]): string[][] => {
  ...
  const splitAt = getSplitPoints(wordCount);
      ...
      <AnimatePresence mode="wait">
        ...
        {appState === AppState.READY && (
          <motion.div key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative w-full">
            ...
            <section id="studio-work" ref={portfolioRef} className="relative min-h-[100vh] md:h-[100vh] w-full bg-black overflow-x-auto overflow-y-auto md:overflow-y-hidden flex items-center z-20">
            ...
            <section id="about" className="relative h-auto z-30 w-full overflow-hidden bg-black">
              <div className="relative z-10 max-w-7xl w-full px-4 sm:px-6 py-8 sm:py-16 md:px-8 md:py-24 lg:pl-32 overflow-hidden pr-[45%] sm:pr-[40%] md:pr-[38%] lg:pr-[35%] xl:pr-[30%]">
                <TornEdge color="rgba(0,0,0,0.8)" />

                <div className="text-left pointer-events-auto flex-1 min-w-0">
                  <h2 className="font-impact text-[clamp(1.75rem,7vw,4rem)] md:text-[clamp(3rem,10vw,8rem)] text-white leading-[0.9] md:leading-none uppercase mb-2 md:mb-4 break-words">CRAFTED <span className="block text-[#E0A9C5]">IN CHAOS</span></h2>
                  <p className="text-zinc-300 font-light text-[clamp(0.65rem,1.2vw,1rem)] leading-relaxed max-w-[180px] sm:max-w-[200px] md:max-w-[280px]">{siteContent?.about?.description || "We don't just cut hair; we engineer identities. The Parlour is a sanctuary for those who seek to bridge the gap between their vision and reality."}</p>
                </div>
                <motion.div
                  ref={aboutCardRef}
                  ...
                  viewport={{ once: true }}
                  className="pointer-events-auto absolute right-0 top-[35%] sm:top-[30%] lg:top-[15%] z-[50] flex items-center"
                >
                  <TiltedCard
                    ...
                    containerWidth="auto"
                    imageHeight="clamp(180px, 35vw, 500px)"
                    imageWidth="clamp(150px, 30vw, 420px)"
                    rotateAmplitude={12}
                    scaleOnHover={1.08}
                    ...
                    overlayContent={
                      siteContent?.about?.imageText1 ? (
                        <p className="font-impact text-sm md:text-xl lg:text-2xl text-white uppercase tracking-wider text-center">
                          {siteContent.about.imageText1}
                        </p>
                      ) : undefined
                    }
            ...
            <footer ref={footerRef} className="premium-footer relative z-[25] bg-white w-full h-[20vh] sm:h-[30vh] md:h-[50vh] flex flex-col justify-end overflow-hidden border-t border-zinc-200 p-0 m-0">
              <div className="footer-top absolute top-0 left-0 w-full z-40 p-3 sm:p-4 md:pl-6 md:pr-8 lg:pl-6 lg:pt-2 xl:p-8 flex flex-row justify-between items-start text-black">
                <div className="flex flex-col gap-1 font-mono text-[9px] sm:text-[9px] md:text-[10px] tracking-widest uppercase">
                  <span>{siteContent?.footer?.copyright_text || '© 2024 THE PARLOUR. ALL RIGHTS RESERVED.'}</span>
                  <span className="opacity-70 normal-case">dev: {siteContent?.footer?.dev_email || 'studio@parlour.com'}</span>
                  <span className="opacity-90 font-bold border-t border-black/10 pt-0.5 normal-case">contact with us: {siteContent?.footer?.contact_email || 'info@theparlour.com'}</span>
                </div>
                <div className="flex gap-2 sm:gap-3 md:gap-6 items-center sm:items-start">
                  <a href={siteContent?.footer?.instagram || '#'} target="_blank" rel="noopener noreferrer" className="hover:scale-110 transition-transform duration-300 group">
                    <svg className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 fill-current text-black group-hover:text-red-600" viewBox="0 0 24 24">
